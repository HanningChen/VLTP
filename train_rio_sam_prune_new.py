...
from model.segment_anything import build_sam_vit_h, build_sam_prune_vit_h, build_sam_prune_vit_b, build_sam_prune_vit_l
...
def parse_args(args):
    ...
    parser.add_argument("--vit_type",default="vit-b", type=str)
    parser.add_argument("--test_model_bin", action="store_true", default=False)
...
def main(args):
    ...
    #NOTE: build SAM
    if args.vit_type == 'vit-b':
        model = build_sam_prune_vit_b(checkpoint=args.vision_pretrained)
    elif args.vit_type == 'vit-h':
        model = build_sam_prune_vit_h(checkpoint=args.vision_pretrained)
    elif args.vit_type == 'vit-l':
        model = build_sam_prune_vit_l(checkpoint=args.vision_pretrained)

    if not args.test_model_bin:
        ...
        DeepsSpeed_bin_path = os.path.join(args.sam_component_dir, 'pytorch_model.bin')
        if not os.path.exists(DeepsSpeed_bin_path):
            print("Please make sure Deepspeed bin file exists")
            exit()
        else:
            with open(DeepsSpeed_bin_path, "rb") as f:
                state_dict = torch.load(DeepsSpeed_bin_path)
    ...
    model.image_encoder.requires_grad = True
